...
function generateExpression(spec) {
  function genNode(depth) {
    if (depth === 0) return { kind: "num", value: randInt(1, spec.maxNum) };
...
function generateProblem(grade) {
  ...
  if (!spec) throw new Error(`No spec for grade ${grade}`);
  return {
...
// ── DEMO WRAPPER (remove when integrating into your game) ─────────────────
export function ProblemOverlayDemo() {
  const [grade, setGrade] = useState(1);
  const [open, setOpen] = useState(false);
  const [log, setLog] = useState([]);

  function handleSolved(problemId, correct, grade) {
    setLog((l) => [
      `#${problemId.slice(0, 8)} | GR.${grade} | ${correct ? "PASS" : "FAIL"}`,
      ...l.slice(0, 9),
    ]);
  }
  ...
  const demoStyles = `
    .demo-root {
      min-height: 100vh;
      background: repeating-linear-gradient(0deg, rgba(200,164,86,0.025) 0px, rgba(200,164,86,0.025) 1px, transparent 1px, transparent 16px),
        repeating-linear-gradient(90deg, rgba(200,164,86,0.025) 0px, rgba(200,164,86,0.025) 1px, transparent 1px, transparent 16px), #0d0a06;
      display: flex; flex-direction: column; align-items: center; justify-content: center;
      font-family: 'Press Start 2P', monospace; gap: 20px; padding: 40px 20px;
      color: #c8a456;
    }
    .demo-title { font-size: 14px; text-shadow: 1px 1px 0 #9a7a38; letter-spacing: 2px; text-align: center; line-height: 2; }
    .demo-sub { font-size: 7px; color: #8a7050; letter-spacing: 3px; margin-top: 4px; }
    .demo-grade-row { display: flex; gap: 8px; }
    .demo-grade-btn { font-family: 'Press Start 2P', monospace; font-size: 8px; padding: 10px 16px;
      cursor: pointer; border: 3px solid #5a4020; background: #1e1508; color: #8a7050;
      box-shadow: 3px 3px 0 0 #0d0a06; transition: all 0.06s; }
    .demo-grade-btn:hover { border-color: #7a5a30; color: #e8dcc0; }
    .demo-grade-btn.active { background: rgba(200,164,86,0.15); border-color: #c8a456; color: #c8a456; box-shadow: 3px 3px 0 0 #9a7a38; }
    .demo-log { background: #120e04; border: 3px solid #5a4020; padding: 12px 16px; min-width: 320px;
      max-height: 160px; overflow-y: auto; }
    .demo-log-entry { font-size: 7px; color: #8a7050; padding: 4px 0; border-bottom: 1px solid #2a1e0e; }
    .demo-log-entry.pass { color: #78b840; }
    .demo-log-entry.fail { color: #e85030; }
    .demo-empty { font-size: 7px; color: #5a4020; text-align: center; padding: 12px; }
  `;

  return (
    <>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Press+Start+2P&family=VT323&display=swap'); ${demoStyles}`}</style>
      <div className="demo-root">
        <div>
          <div className="demo-title">NUMBER RAIDERS</div>
          <div className="demo-sub">PROBLEM OVERLAY · DEMO</div>
        </div>
        ...
        <div className="demo-grade-row">
          {[1, 2, 3].map((g) => (
            <div
              key={g}
              className={`demo-grade-btn ${grade === g ? "active" : ""}`}
              onClick={() => setGrade(g)}
            >
              GRADE {g}
            </div>
        ...
        <div className="demo-log">
          ...
          {log.length === 0 ? (
            <div className="demo-empty">No problems attempted yet...</div>
          ) : (
            log.map((entry, i) => (
              <div
                key={i}
                className={`demo-log-entry ${entry.includes("PASS") ? "pass" : "fail"}`}
              >
                {entry}
              </div>
            ))
